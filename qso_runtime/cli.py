...
def _package_version() -> str:
    """Return installed metadata, falling back to source-tree metadata."""
    try:
...
def _self_check() -> dict[str, object]:
    """Return a deterministic declaration of the current runtime boundary."""
    return {
        "boundaries": {
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--pretty",
        action="store_true",
        help="indent the deterministic self-check JSON",
    )
    return parser
